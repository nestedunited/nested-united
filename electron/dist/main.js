...
async function startNextServer() {
    return new Promise((resolve, reject) => {
        ...
        nextServerProcess = (0, child_process_1.spawn)("node", [serverPath], {
            env,
            cwd,
            shell: true,
        });
    ...
    const menu = electron_1.Menu.buildFromTemplate([
        {
            ...
            submenu: [
                ...
                { type: "separator" },
                { label: "خروج", accelerator: "CmdOrCtrl+Q", click: () => { isAppQuitting = true; electron_1.app.quit(); } },
            ],
        },
        {
            label: "عرض",
...
function createBrowserWindow(accountSession) {
    ...
    const platformUrl = accountSession.platform === "airbnb"
...
function injectNotificationMonitor(browserWindow, account) {
...
// IPC Handlers
electron_1.ipcMain.handle("get-browser-accounts", () => {
...
electron_1.ipcMain.handle("add-browser-account", (_, account) => {
...
electron_1.ipcMain.handle("remove-browser-account", (_, accountId) => {
    ...
    electron_1.session.fromPartition(partition).clearStorageData();
    return { success: true };
});
electron_1.ipcMain.handle("open-browser-account", (_, accountId) => {
    const account = browserSessions.get(accountId);
    if (!account)
...
electron_1.ipcMain.handle("close-browser-account", (_, accountId) => {
    const account = browserSessions.get(accountId);
    if (account?.window && !account.window.isDestroyed()) {
        account.window.close();
        account.window = undefined;
...
electron_1.ipcMain.handle("browser-go-back", (_, accountId) => {
    const account = browserSessions.get(accountId);
    if (account?.window && !account.window.isDestroyed()) {
        account.window.webContents.goBack();
    }
});
electron_1.ipcMain.handle("browser-go-forward", (_, accountId) => {
    const account = browserSessions.get(accountId);
    if (account?.window && !account.window.isDestroyed()) {
        account.window.webContents.goForward();
    }
});
electron_1.ipcMain.handle("browser-reload", (_, accountId) => {
    const account = browserSessions.get(accountId);
    if (account?.window && !account.window.isDestroyed()) {
        account.window.webContents.reload();
    }
});
electron_1.ipcMain.handle("browser-go-home", (_, accountId) => {
    const account = browserSessions.get(accountId);
    if (account?.window && !account.window.isDestroyed()) {
...
electron_1.ipcMain.on("new-notification-from-webview", (_, data) => {
    console.log("[Main Process] Received notification from webview:", data);
    const platformName = data.platform === "airbnb" ? "Airbnb" : "Gathern";
    showSystemNotification(`إشعار جديد على ${platformName}`, `يوجد إشعار جديد على الحساب: ${data.accountName}`);
...
electron_1.ipcMain.on("database-notification", (_, data) => {
...
electron_1.ipcMain.handle("focus-tab", (_, accountId) => {
    const account = browserSessions.get(accountId);
    if (account?.window && !account.window.isDestroyed()) {
